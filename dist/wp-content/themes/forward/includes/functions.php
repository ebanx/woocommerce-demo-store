...
function dh_serch_button_icon(){
	return apply_filters('dh_serch_button_icon', '<img src="'.get_template_directory_uri().'/assets/images/icon-search.png" alt="'.esc_attr__('Search','forward').'">');
}

function dh_user_icon(){
	return apply_filters('dh_user_icon', '<img src="'.get_template_directory_uri().'/assets/images/icon-user.png" alt="'.esc_attr__('User','forward').'">');
}
	...
	<div class="language-switcher">
		...
		if( is_array( $languages ) ){
			...
			if($type == 'select'){
				...
				if($label){
					echo '<label>'.__('Languages','forward').'</label>';
				}
				if( count( $languages ) ){
...
function dh_instagram($username,$images_number=12,$refresh_hour){
	if (false === ($instagram = get_transient('instagram-'.sanitize_title_with_dashes($username.$images_number)))) {
		...
		if ( is_wp_error( $remote ) )
			return new WP_Error( 'site_down',__( 'Unable to communicate with Instagram.', 'forward' ));
	
		if ( 200 != wp_remote_retrieve_response_code( $remote ) )
			return new WP_Error( 'invalid_response',__( 'Instagram did not return a 200.', 'forward' ));
		...
		if ( !$insta_array )
			return new WP_Error( 'bad_json', __( 'Instagram has returned invalid data.', 'forward' ) );
		...
		switch ( $type ) {
			case 'old':
				foreach ( $images as $image ) {
					...
					if ( $image['user']['username'] == $username ) {
						...
						$instagram[] = array(
							'description'   => apply_filters('dh_instagram_image_description',__('New arrivals finally here #s/s 2016','forward'),$image), //$image['caption']['text']
							'link'		  	=> $image['link'],
			...
			default:
				foreach ( $images as $image ) {
					...
					$instagram[] = array(
						'description'   => apply_filters('dh_instagram_image_description',__('New arrivals finally here #s/s 2016','forward'),$image),
...
function dh_get_theme_option($option,$default = null){
	...
	if(is_page() || (defined('WOOCOMMERCE_VERSION') && is_woocommerce())){
		...
		if($option == 'sticky-menu'){
			$page_value = dh_get_post_meta('sticky-menu');
			if(!is_null($page_value) && $page_value != '-1'){
				return apply_filters('dh_get_theme_option', $page_value, $option);
			}
		}
		if($option == 'footer-top'){
			$page_value = dh_get_post_meta('footer_top');
			if(!is_null($page_value) && $page_value != '-1'){
				return apply_filters('dh_get_theme_option', $page_value, $option);
			}
		}
		if($option == 'footer-bottom'){
			$page_value = dh_get_post_meta('footer_bottom');
			if(!is_null($page_value) && $page_value != '-1'){
				return apply_filters('dh_get_theme_option', $page_value, $option);
			}
		}
		if($option == 'footer-info-switch'){
			$page_value = dh_get_post_meta('footer_info_switch');
			if(!is_null($page_value) && $page_value != '-1'){
				return apply_filters('dh_get_theme_option', $page_value, $option);
			}
		}
		...
		if($option == 'footer-style'){
			$page_value = dh_get_post_meta('footer-style');
			if(!is_null($page_value) && $page_value != '-1'){
				return apply_filters('dh_get_theme_option', $page_value, $option);
			}
		}
...
<div class="related-post">
	<div class="related-post-title">
		<h3><span><?php echo __("Related News",'forward')?></span></h3>
	</div>
		...
		<?php while ($related->have_posts()): $related->the_post();global $post;?>
			<div class="related-post-item col-sm-6">
				...
				<div class="readmore-link">
					<a href="<?php the_permalink()?>"><?php _e("Read", 'forward');?></a>
				</div>
			</div>
		<?php endwhile;?>
	</div>
...
function dh_list_comments($comment, $args, $depth) {
	global $post;
	$avatar_size = isset($args['avatar_size']) ? $args['avatar_size'] : 68;
	?>
					...
					<header class="comment-header">
						<div class="comment-info">
							...
							<span class="ispostauthor">
								<?php _e('Author', 'forward'); ?>
							</span>
							<?php endif; ?>
							<span class="comment-meta">
								<time datetime="<?php echo get_comment_time('c'); ?>">
									<?php echo sprintf(__('%1$s', 'forward') , get_comment_date('M d, Y')); ?>
								</time>
						...
						<?php if ('0' == $comment->comment_approved): ?>
							<p class="comment-pending"><?php _e('Your comment is awaiting moderation.', 'forward'); ?></p>
						<?php endif; ?>
					</header>
...
function dh_get_search_form(){
	...
	$search_form = '<form method="GET" class="searchform'.($ajax ?' search-ajax':'').'" action="'.esc_url( home_url( '/' ) ).'" role="form">
					<input type="search" class="searchinput" name="s" autocomplete="off" value="" placeholder="'.__( 'Search...', 'forward' ).'" />
					<input type="submit" class="searchsubmit hidden" name="submit" value="'.__( 'Search', 'forward' ).'" />
...
function dh_comment_form( $args = array(), $post_id = null ) {
	...
	<div id="respond-wrap">
		<?php 
			...
			$fields =  array(
				'author' => '<div class="row"><p class="comment-form-author col-sm-6">' .'<input placeholder="'.__('Your Name *','forward').'" id="author" name="author" type="text" class="form-control" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>',
				'email' => '<p class="comment-form-email col-sm-6">'.'<input id="email" placeholder="'.__('Your Email *','forward').'" name="email" type="text" class="form-control" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></p></div>'
			);
			$comments_args = array(
					'fields'               => apply_filters( 'comment_form_default_fields', $fields ),
					'logged_in_as'		   => '<p class="logged-in-as">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', 'forward' ), admin_url( 'profile.php' ), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( ) ) ) ) . '</p>',
					'title_reply'          => sprintf('<span>%s</span>',__( 'Leave a reply', 'forward' )),
					'title_reply_to'       => sprintf('<span>%s</span>',__( 'Leave a reply to %s', 'forward' )),
					'cancel_reply_link'    => __( 'Click here to cancel the reply', 'forward' ),
					'label_submit'         => __( 'Post', 'forward' ),
					'comment_field'		   => '<p class="comment-form-comment"><textarea placeholder="'.__('Your Message','forward').'"  class="form-control" id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>',
					'must_log_in'		   => '<p class="must-log-in">' .  sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.', 'forward' ), wp_login_url( apply_filters( 'the_permalink', get_permalink( ) ) ) ) . '</p>',
			);
...
function dh_nth_word($text, $nth = 1, $echo = true,$is_typed = false,$typed_color = ''){
	...
	if ( 'characters' == _x( 'words', 'word count: words or characters?','forward') && preg_match( '/^utf\-?8$/i', get_option( 'blog_charset' ) ) ) {
...
function dh_post_nav() {
	...
	<nav class="post-navigation" role="navigation">
		<div class="row">
			<?php $prev_link = get_previous_post_link( '%link', _x( '%title', 'Previous post link', 'forward' ) ); ?>
			<div class="col-sm-6">
			<?php if($prev_link):?>
				<div class="prev-post">
					<span>
					<?php echo __('Previous article','forward')?>
					</span>
					<?php echo dh_print_string($prev_link)?>
				</div>
			<?php endif;?>
			</div>
			<?php $next_link = get_next_post_link( '%link', _x( '%title', 'Next post link', 'forward' ) ); ?>
			<div class="col-sm-6">
			<?php if(!empty($next_link)):?>
				<div class="next-post">
					<span>
						<?php echo __('Next article','forward')?>
					</span>
...
function dh_page_title($echo = true){
	...
	elseif (is_day())
	{
		$title = __('Archive for date:','forward')." ".get_the_time('F jS, Y');
	}
	elseif (is_month())
	{
		$title = __('Archive for month:','forward')." ".get_the_time('F, Y');
	}
	elseif (is_year())
	{
		$title = __('Archive for year:','forward')." ".get_the_time('Y');
	}
	elseif (is_search())
	{
		global $wp_query;
		if(!empty($wp_query->found_posts))
		{
			...
			{
				$title =  $wp_query->found_posts ." ". __('search results for','forward').' &ldquo;'.esc_attr( get_search_query() ).'&rdquo;';
			}
			else
			{
				$title =  $wp_query->found_posts ." ". __('search result for','forward').' &ldquo;'.esc_attr( get_search_query() ).'&rdquo;';
			}
		}
		else
		{
			if(!empty($_GET['s']))
			{
				$title = __('Search results for','forward').' &ldquo;'.esc_attr( get_search_query() ).'&ldquo;';
			}
			else
			{
				$title = __('To search the site please enter a valid term','forward');
			}
		}
	...
	elseif(is_tax())
	{
	    ...
	    $title = __('404 - Page not found','forward');
	}
	else {
		$title = get_the_title();
	}
	...
	{
		$title .= " (".__('Page','forward')." ".$_GET['paged'].")";
	}
...
// 		}
	...
	if(is_post_type_archive( 'portfolio' )){
		$title = esc_html(dh_get_theme_option('portfolio-archive-title',__('My Portfolio','forward')));
	}
	if($echo)
...
function dh_post_featured($post_id='',$post_format='',$is_shortcode = false,$is_related = false,$entry_featured_class = '',$layout = '',$is_thumb_square=false){
	...
	if($layout == 'masonry'){
		$thumb_size = 'dh-masonry-thumbnail';
	}
	if($layout == 'zigzag' || $is_thumb_square){
		$thumb_size = 'dh-thumbnail-square-small';
	}
	...
	if($is_related){
		...
		if($post_format == 'gallery'){
			...
			if (has_post_thumbnail()){
				...
				echo '</div>';
			}elseif ($layout == 'zigzag' || $layout == 'masonry' || $layout == 'grid'){
...
function dh_post_meta($show_date=true,$show_comment = true,$show_category= true,$show_author = true,$echo = true,$meta_separator= ', ',$date_format = null,$icon = false) {
	...
	if($show_author){
		...
		if($icon)
			$author_html .= '<i class="fa fa-user"></i>';//__('By', 'forward');
		$author = sprintf(
			'<a href="%1$s" title="%2$s" rel="author">%3$s</a>',
			esc_url( get_author_posts_url( get_the_author_meta( 'ID' ), get_the_author_meta( 'nicename' ) ) ),
			esc_attr( sprintf( __( 'Posts by: %s', 'forward'), get_the_author() ) ),
			get_the_author()
		);
		$author_html .= sprintf(__('%1$s', 'forward'),$author);
		$author_html .= '</span>';
		$html[] = $author_html;
	}
	// Categories
	...
	if($show_category){
		...
		$categories_html .= sprintf(__('%1$s','forward'),get_the_category_list(', '));
	...
	if (comments_open()) {
		...
		if (get_comments_number() == 0) {
			$comment_title = sprintf(__('Leave a comment on: &ldquo;%s&rdquo;', 'forward') , get_the_title());
			$comment_number = '0';
			$comment_text = __('Comment','forward');
		} else if (get_comments_number() == 1) {
			$comment_title = sprintf(__('View a comment on: &ldquo;%s&rdquo;', 'forward') , get_the_title());
			$comment_number = '1';
			$comment_text = __('Comment','forward');
		} else {
			$comment_title = sprintf(__('View all comments on: &ldquo;%s&rdquo;', 'forward') , get_the_title());
			$comment_number =  get_comments_number();
			$comment_text = __('Comments','forward');
		}
...
function dh_paginate_links_short($args = array(), $query = null){
	...
	$defaults = array(
		...
		'add_fragment' => '',
		'add_args'=>array(),
		'before' => '<div class="paginate"><div class="paginate_links"><span class="pagination-meta">'.sprintf(__("%d/%d", 'forward'), $paged, $max_num_pages).'</span>',
		'after' => '</div></div>',
		'echo' => true,
	);
...
function dh_paginate_next_prev($args = array(), $query = null){
	...
	<div class="paginate">
		<div class="paginate_links paginate_next_prev clearfix">
			...
			<div class="nav-previous pull-left"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'forward' ) ); ?></div>
			<?php endif; ?>

			<?php if ( get_previous_posts_link() ) : ?>
			<div class="nav-next pull-right"><?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'forward' ) ); ?></div>
			<?php endif; ?>
		</div>
	</div>
	<?php
...
function dh_paginate_links( $args = array(), $query = null ){
	...
	if( $wp_rewrite->using_permalinks() && ! is_search() )
		$defaults['base'] = user_trailingslashit( trailingslashit( get_pagenum_link(1,false) ) . 'page/%#%' );
...
function dh_morphsearchform(){
	...
	<div class="morphsearch" id="morphsearch">
		<form class="morphsearch-form" method="get"  action="<?php echo esc_url( home_url( '/' ) ) ?>" role="form">
			<input type="search" name="s" placeholder="<?php echo esc_attr__('Search...','forward')?>" class="morphsearch-input">
		...
		<div class="morphsearch-content<?php echo (defined( 'WOOCOMMERCE_VERSION' )  ? ' has-3colum':'') ?>">
			...
			<div class="dummy-column">
				...
				<a href="<?php the_permalink()?>" class="dummy-media-object">
					...
					<div>
						<h3><?php the_title()?></h3>
						<?php echo '<span>'.sprintf(__('%s Comment','forward'),get_comments_number()).'</span>'; ?>
					</div>
				</a>
				<?php endwhile; ?>
				<?php 
	...
	<form id="mailchimp-form-<?php echo wp_create_nonce('mailchimp_subscribe_nonce')?>" method="get" class="mailchimp-form" action="<?php echo home_url('/')?>">
		...
		<div class="mailchimp-form-result">
			...
			if($mailchimp_subscribe == -1){
				echo '<span class="error">'.__( 'Not Subscribe to Mailchimp!', 'forward' ).'<span>';
			}elseif ($mailchimp_subscribe == 1){
				echo '<span class="success">'.__( 'Subscribe to Mailchimp Successful!', 'forward' ).'</span>';
			}
			?>
		</div>
		<?php endif;?>
		<div class="mailchimp-form-content clearfix">
			...
			<label for="" class="hide"><?php _e('Subscribe','forward')?></label>
			<input type="email" required="required" autocomplete="off" placeholder="<?php echo esc_attr__('Your email...','forward')?>" name="email">
		
			<button type="submit" class="btn mailchimp-submit"><?php _e('Submit','forward')?></button>
