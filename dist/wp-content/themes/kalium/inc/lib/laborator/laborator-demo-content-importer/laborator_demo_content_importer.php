...
function lab_1cl_demo_installer_get_packs() {
	return array(
		...
		array(
			...
			'url'			 => 'http://demo.kaliumtheme.com/fashion'
		),
...
function lab_1cl_demo_installer_admin_init() {
	...
	if ( kalium()->get( 'page' ) == 'laborator-demo-content-installer' && ( $pack_name = kalium()->get( 'install-pack' ) ) ) {
...
function lab_1cl_demo_install_package_content() {
	...
	$pack_name = kalium()->get( 'pack' );
	$source_details = kalium()->get( 'contentSourceDetails' );
	...
	switch ( $source_details['type'] ) {
		...
		case "typolab":
			...
			$typolab = maybe_unserialize( file_get_contents( $typolab ) );
