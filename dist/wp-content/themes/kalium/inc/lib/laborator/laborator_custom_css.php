...
function laborator_custom_css_page() {
	...
	$codemirror_path = kalium()->locateFileUrl( 'inc/lib/laborator/custom-css-lib/codemirror-4.7/' );
	...
	if ( kalium()->get( 'tab' ) == 'responsive' ) {
		$current_tab = 'responsive';
	}

	if ( kalium()->get( 'tab' ) == 'less' ) {
		$current_tab = 'less';
	}

	if ( kalium()->get( 'tab' ) == 'sass' ) {
		$current_tab = 'sass';
	}
?>
<!-- CodeMirror -->
...
<div class="wrap">
	...
	<h2 class="nav-tab-wrapper woo-nav-tab-wrapper">
		<a href="?page=<?php echo kalium()->get( 'page' ); ?>&tab=main" class="nav-tab<?php echo $current_tab == 'main' ? ' nav-tab-active' : ''; ?>">General Style</a>
		<a href="?page=<?php echo kalium()->get( 'page' ); ?>&tab=responsive" class="nav-tab<?php echo $current_tab == 'responsive' ? ' nav-tab-active' : ''; ?>">Responsive</a>
		<a href="?page=<?php echo kalium()->get( 'page' ); ?>&tab=less" class="nav-tab<?php echo $current_tab == 'less' ? ' nav-tab-active' : ''; ?>">LESS</a>
		<a href="?page=<?php echo kalium()->get( 'page' ); ?>&tab=sass" class="nav-tab<?php echo $current_tab == 'sass' ? ' nav-tab-active' : ''; ?>">SASS</a>
...
// Export
if ( kalium()->get( 'lab-custom-css-export' ) && current_user_can( 'manage_options' ) ) {
	$custom_css    = laborator_get_custom_css();
	$custom_css_lg = laborator_get_custom_css( 'lg' );
	$custom_css_md = laborator_get_custom_css( 'md' );
	$custom_css_sm = laborator_get_custom_css( 'sm' );
