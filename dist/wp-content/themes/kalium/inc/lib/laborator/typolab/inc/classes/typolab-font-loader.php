...
class TypoLab_Font_Loader extends TypoLab {
	...
	public function enqueueFontURL( $font_url, $font_placement ) {
		...
		// Enqueue font before footer
			...
			$font_import_fn = create_function( '', 'wp_enqueue_style( "' . addslashes( $font_id ). '", "' . addslashes( $font_url ) . '", null, "' . addslashes( $this->checksum ). '" );' );
	...
	public function combineFonts( $fonts ) {
		...
		if ( isset( $font_import_files[ $file_id ] ) ) {
	...
	public function enqueueFontsFromTypeKit( $fonts, $font_placement ) {
		...
		foreach ( $fonts as $font ) {
			...
			if ( $kit_id ) {
				$font_import_fn = create_function( '', 'TypoLab_TypeKit_Fonts::embedKitJS( "' . addslashes( $kit_id ) . '" );' );
	...
	private function createFontVariantsSelectorsAndFontSizes( $variants ) {
		...
		// Defined Font Sizes
		$general_sizes = $desktop_sizes = $tablet_sizes = $mobile_sizes = array();
