...
class TypoLab {
	...
	public function __construct() {
		...
		// TypoLab Assets URL
		self::$typolab_assets_url = site_url( str_replace( ABSPATH, '', self::$typolab_path ) . '/assets/' );
	...
	public function typoLabAdminInit() {
		...
		if ( 'typolab' == kalium()->get( 'page' ) ) {
			wp_enqueue_style( array( 'typolab-main', 'font-awesome' ) );
			...
			$action = kalium()->get( 'typolab-action' );
			$font_id = kalium()->get( 'font-id' );
			...
			// Delete Custom Font Sizes Group
				...
				$delete_group_id = kalium()->get( 'group-id' );
			...
			// Font Sizes Page
			if ( 'font-sizes' == kalium()->get( 'typolab-page' ) ) {
				// Save Font Sizes
			...
			// Font Settings Page
			else if ( 'settings' == kalium()->get( 'typolab-page' ) ) {
				
				// Delete Font Files
				if ( $downloaded_font = kalium()->get( 'delete-font-files' ) ) {
				...
				if ( kalium()->post( 'save_font_settings' ) && check_admin_referer( 'typolab-save-font-settings' ) ) {
					...
					if ( ! empty( $import_font_settings ) ) {
						$import_font_settings = maybe_unserialize( wp_unslash( $import_font_settings ) );
		...
		if ( self::$missing_fonts && 'typolab' !== kalium()->get( 'page' ) ) {
	...
	private function saveFontChanges() {
		...
		// Font Selectors
		$font['options']['selectors'] = self::preserveSelectorsOrder( $font_selectors );
	...
	public function fontExportImportManager() {
		...
		if ( kalium()->post( 'doExport' ) ) {
			...
			$resp['exported'] = maybe_serialize( $export_import_manager->export( $font_faces, $font_sizes, $font_settings ) );
		}
	...
	public function typographyMenuItem() {
		...
		if ( ! is_null( kalium()->post( 'typolab_add_font' ) ) && check_admin_referer( 'typolab-add-font' ) ) {
			...
			if ( in_array( $font_source, $font_sources_ids ) ) {
				...
				$url = admin_url( '"admin.php?page=' . kalium()->get( 'page' ) . '&typolab-action=edit-font&font-id=' . $font_id );
	...
	public function getFonts( $valid_fonts_only = false, $published_fonts_only = false ) {
		...
		uasort( $fonts, array( & $this, 'sortFontsBySource' ) );
	...
	public function sortFontsBySource( $a, $b ) {
	...
	public function typoLabPage() {
		...
		switch ( $page ) {
			...
			default:
				...
				// Edit Font
				else if ( 'edit-font' == $action && ( $font_id = kalium()->get( 'font-id' ) ) ) {
					$font = self::getFont( $font_id );
					if ( $font ) {
						$title = 'Edit Font';
