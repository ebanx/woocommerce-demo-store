...
class TypoLab_Font_Export_Import {
	...
	public function importFontFaces( $font_faces ) {
		...
		foreach ( $font_faces as $i => $font_face ) {
			...
			// Check existing font face if already exists
			foreach ( $current_font_faces as $j => $current_font_face ) {
				// Replace existing font face
				if ( $font_face == $current_font_face ) {
					$exists = true;
					break;
				}
			}
	...
	public function importFontSizes( $font_sizes ) {
		...
		if ( isset( $font_sizes['customFontSizeGroups'] ) ) {
