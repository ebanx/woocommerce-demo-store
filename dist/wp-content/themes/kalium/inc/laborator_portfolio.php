...
function kalium_portfolio_item_type_filter_dropdown() {
	...
	if ( $pagenow == 'edit.php' && $typenow == 'portfolio' ) {
		...
		$current_item_type = kalium()->get( 'portfolio_item_type' );
...
function kalium_portfolio_item_type_filter_query_request( $query ) {
	...
	$item_type = kalium()->get( 'portfolio_item_type' );
...
function kalium_portfolio_generate_gallery_gap( $gap = 30, $type = '' ) {
	...
	if ( $type == 'carousel' ) {
		...
		generate_custom_style( '.single-portfolio-holder .gallery .photo, .single-portfolio-holder .portfolio-video-holder, .single-portfolio-holder .gallery .comparison-image-slider', "margin-bottom:{$gap}px;" );
