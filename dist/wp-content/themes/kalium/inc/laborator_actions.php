...
function laborator_init() {
	...
	wp_register_script( 'joinable', kalium()->assetsUrl( 'js/joinable.min.js' ), null, $version, true );
	
	wp_register_script( 'custom', kalium()->assetsUrl( 'js/custom.min.js' ), null, $version, true );
...
if ( file_exists( $theme_demo_file ) && is_readable( $theme_demo_file ) && version_compare( PHP_VERSION, '5.3.0', '>=' ) ) {
...
function laborator_widgets_init() {
	...
	register_sidebar( array_merge( $blog_sidebar, array( 
		'id' => 'blog_sidebar_single',
		'name' => 'Single Blog Widgets'
	) ) );
	...
	// Sidebar on single post
	register_sidebar( array_merge( $shop_sidebar, array( 
		'id' => 'shop_sidebar_single',
		'name' => 'Single Product Widgets'
	) ) );
...
function kalium_register_required_plugins() {
	...
	$message = '<p style="color: #888">If any of theme required plugins has new update after <strong>' . THEMENAME . ' ' . kalium()->getVersion() .  '</strong> was released, we will include the latest version of that plugin in the next theme update.</p>';
...
function laborator_get_paged_blog_posts() {
	...
	// Import blog options
	include_once kalium()->locateFile( 'tpls/blog-query.php' );
...
function laborator_coming_soon_mode() {
	...
	if ( $coming_soon_mode && $manage_options == false || kalium()->get( 'view-coming-soon' ) ) {
		get_template_part( 'coming-soon-mode' );
		die();
	}

	if ( $maintenance_mode && $manage_options == false || kalium()->get( 'view-maintenance' ) ) {
...
if ( function_exists( 'set_revslider_as_theme' ) ) {
	...
	// Remove RevSlider notice
	if ( 'true' == get_data( 'revslider-valid-notice', 'true' ) ) {
...
function check_for_custom_logo_in_page() {
	...
	if ( $custom_sticky_logo ) {
		add_filter( 'get_data_sticky_header_logo', laborator_immediate_return_fn( $custom_sticky_logo ) );
	}
	...
	if ( get_data( 'sticky_header' ) && $sticky_menu_skin && 'default' !== $sticky_menu_skin ) {
		add_filter( 'get_data_sticky_header_skin', laborator_immediate_return_fn( $sticky_menu_skin ) );
	}
}
...
function laborator_holidays_wishes_css() {
	...
	if ( $pagenow == 'themes.php' && kalium()->get( 'page' ) == 'theme-options' ) {
		return;
	}
...
function kalium_font_size_groups() {
	...
	$widgets_title = '.top-menu-container .sidebar-box-holder h3';
	$widgets_text  = '.top-menu-container .sidebar-box-holder, .top-menu-container .sidebar-box-holder p, .top-menu-container .sidebar-box-holder div';
	...
	$widgets_title = '.sidebar-menu-wrapper .sidebar-menu-container .sidebar-menu-widgets .sidebar-box-holder .sidebar-entry-title';
	$widgets_text  = '.sidebar-menu-wrapper .sidebar-box-holder, .sidebar-menu-wrapper .sidebar-box-holder p, .sidebar-menu-wrapper .sidebar-box-holder div';
	...
	// Blog
	$post_title_loop	= '.blog-holder .box-holder .post-info h2, .wpb_wrapper .lab-blog-posts .blog-post-entry .blog-post-content-container .blog-post-title';
	...
	// Footer
	$widgets_title = '.main-footer .footer-widgets .wp-widget h1, .main-footer .footer-widgets .wp-widget h2, .main-footer .footer-widgets .wp-widget h3';
	$widgets_text  = '.main-footer .footer-widgets .wp-widget .textwidget, .main-footer .footer-widgets .wp-widget p';
	$copyrights    = '.copyrights, .main-footer .footer-bottom-content a, .main-footer .footer-bottom-content p';
...
function kalium_theme_is_installed_hook() {
	$kalium_installed_var = 'kalium_installed';
	
	if ( false == get_option( $kalium_installed_var, false ) ) {		
		update_option( $kalium_installed_var, true );
		wp_redirect( admin_url( 'admin.php?page=laborator_options&welcome=kalium' ) );
		die();
	} 
	
	delete_option( $kalium_installed_var );
}

add_action( 'after_switch_theme', 'kalium_theme_is_installed_hook' );
