	...
	<div class="laborator-system-status-data">
		...
		<table class="widefat">
			...
			<tbody>
				...
				<tr>
					...
					<?php
						...
						if ( 180 > $time_limit && 0 != $time_limit ) {
							echo '<mark class="error">' . sprintf( '%1$s - We recommend setting max execution time to at least 180.<br />See: <a href="%2$s" target="_blank" rel="noopener noreferrer">Increasing max execution to PHP</a>', $time_limit, 'http://codex.wordpress.org/Common_WordPress_Errors#Maximum_execution_time_exceeded' ) . '</mark>';
						} else {
				...
				<tr>
					<td>MySQL Version:</td>
					<td class="help"><?php Laborator_System_Status::tooltip( 'The version of MySQL installed on your hosting server.' ); ?></td>
					<th>
				...
				<tr>
					...
					<th>
					<?php
						$response = wp_safe_remote_post( 'https://envato.com/', array( 'decompress' => false, 'user-agent' => 'kalium-test-wp-remote-post' ) );
						echo ( ! is_wp_error( $response ) && $response['response']['code'] >= 200 && $response['response']['code'] < 300 ) ? $mark_yes : $mark_no; 
					?>
					</th>
				</tr>
			...
			foreach ( $active_plugins as $plugin ) {
				...
				if ( ! empty( $plugin_data['Name'] ) ) {
					...
					<tr>
						...
						<th class="not-bold"><?php echo sprintf( _x( 'by %s', 'by author', 'woocommerce' ), $plugin_data['Author'] ) . ' &ndash; ' . esc_html( $plugin_data['Version'] ) . $version_string . $network_string; ?></th>
					</tr>
					<?php
				}
			}
